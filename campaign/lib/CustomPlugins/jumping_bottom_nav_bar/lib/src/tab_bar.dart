...
class _JumpingTabBarState extends State<JumpingTabBar> with SingleTickerProviderStateMixin {
  ...
  void didChangeDependencies() {
    ...
    tabWidth = MediaQuery.of(context).size.width / widget.items.length;
  ...
  void onTabCtrlChange() {
    if (tabController.index + 1 != selectedIndex) onChangeTab(tabController.index + 1, shouldCallParent: false);
  }
  ...
  void onChangeTab(int index, {bool shouldCallParent = true}) {
    ...
    if (widget.onChangeTab != null && shouldCallParent) widget.onChangeTab(index);

    double cPoint = (tabWidth - circleSize) / 2;
    var nextPx = (index - 1) * tabWidth + cPoint;
    double nextPy = -15;
